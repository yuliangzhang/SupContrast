...
import time
import logging
from audio_utils import config_esc50
from audio_utils.utilities_esc50 import int16_to_float32
...
class Esc50Dataset(object):
    ...
    def __getitem__(self, meta):
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            ...
            target = hf['target'][index_in_hdf5].astype(np.int)
...
class Base(object):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv, random_seed):
        ...
        with h5py.File(indexes_hdf5_path, 'r') as hf:
            ...
            self.targets = hf['target'][:].astype(np.int)
        
        (self.audios_num, self.classes_num) = self.targets.shape, config_esc50.classes_num
        logging.info('Training number: {}'.format(self.audios_num))
        logging.info('Load target time: {:.3f} s'.format(time.time() - load_time))

    def __len__(self):

        return int(len(self.audio_names) / self.batch_size)


class Esc50TrainSampler(Base):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv=None, 
        ...
        Args:
          ...
          random_seed: int
        """
        super(Esc50TrainSampler, self).__init__(indexes_hdf5_path, batch_size,
...
class Esc50BalancedTrainSampler(Base):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv=None, 
        random_seed=1234):
        ...
        Args:
          ...
          random_seed: int
        """
        super(Esc50BalancedTrainSampler, self).__init__(indexes_hdf5_path,
            batch_size, black_list_csv, random_seed)
        
        # self.samples_num_per_class = np.sum(self.targets, axis=0)
        _, self.samples_num_per_class = np.unique(self.targets, return_counts=True)
...
class Esc50AlternateTrainSampler(Base):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv=None,
        random_seed=1234):
        ...
        Args:
          ...
          random_seed: int
        """
        self.sampler1 = Esc50TrainSampler(indexes_hdf5_path, batch_size,
            black_list_csv, random_seed)

        self.sampler2 = Esc50BalancedTrainSampler(indexes_hdf5_path, batch_size,
            black_list_csv, random_seed)
...
class Esc50EvaluateSampler(object):
    ...
    def __len__(self):
        return int(self.audios_num / self.batch_size)



def esc50_collate_fn(list_data_dict):
    """Collate data.
    Args:
